...
- *TODO*: Make a hydra for =golden-ratio=.
...
(use-package spaceline-config
  ...
  (spaceline-install
   ...
   `(((flycheck-error flycheck-warning flycheck-info) :when active :skip-alternate t)
     ((projectile-root version-control) :separator ":" :skip-alternate t)
     (selection-info :skip-alternate t)
  ...
  (set-face-attribute 'powerline-inactive1 nil :background zenburn/bg-1)
  (set-face-attribute 'powerline-inactive2 nil :background zenburn/bg+0)
  ...
  (set-face-attribute 'spaceline-unmodified nil
                      :background zenburn/green-1 :foreground zenburn/green+4))
#+END_SRC

Brace yourself, for this is quite big and possibly sub-optimal. I use =powerline= to
customize the =mode-line= a la Vim.

- *TODO*: Harvest and filter the following.
...
(use-package powerline
  ...
  (defpowerline me/powerline-vc
    (when (and (buffer-file-name (current-buffer)) vc-mode)
      (format "%s" (format-mode-line '(vc-mode vc-mode)))))
  ...
  (set-face-attribute 'powerline-active1 nil
                      :background zenburn/bg-0 :foreground zenburn/fg)
