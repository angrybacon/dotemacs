...
(use-package olivetti
  ...
  :custom
  (olivetti-body-width (+ fill-column 6)))
#+END_SRC
  ...
  (when me/olivetti-automatic
    (let* ((predicate (lambda (window)
                        (with-selected-window window
                          (or (buffer-file-name)
                              (member (buffer-name)
                                      me/olivetti-whitelist-buffers)
                              (apply 'derived-mode-p
                                     me/olivetti-whitelist-modes)))))
...
(use-package doom-modeline
  ...
  (doom-modeline-bar-inactive
   ((t (:background nil :inherit mode-line-inactive))))
  :config
...
(defvar me/vterm-shackles `(:align 'below
                            :popup t
                            :size ,(szadek-get 'popup-height .33))
