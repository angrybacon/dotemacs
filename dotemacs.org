...
| TODO | Make =xdg-open= switch workspace/eyebrowse automatically |
| TODO | Notifications                                            |
| TODO | Remove =exwm-config-default= dependency                  |
| TODO | Clean up system tray setup                               |
| TODO | Check out https://github.com/agzam/exwm-edit             |
| TODO | Load display setup from =.xinitrc= or =.secret.el=       |
...
(use-package exwm
  ...
  :custom
  (exwm-randr-workspace-monitor-plist '(0 "DVI-D-0" 1 "HDMI-0"))
...
(defun me/exwm-xrandr ()
  "Set resolution for EXWM displays."
  (start-process-shell-command
   "xrandr" nil "xrandr \
--output HDMI-0 --mode 1920x1080 --pos 0x0 \
--output DVI-D-0 --mode 1920x1080 --pos 1920x0"))
#+END_SRC

Some UI elements are rather invasive. No-mouse master race.

| TODO | Move these to =early-init.el= |
...
| TODO | Prevent fullscreen to apply to EXWM sessions |
...
#+BEGIN_SRC emacs-lisp
(defvar me/theme-known-themes '(doom-one doom-nord-light)
  "List of themes to take into account with `me/theme-cycle'.")

(defun me/theme-load-themes (&optional themes)
  "Load `me/theme-known-themes' without enabling them.
Consider THEMES instead if not nil."
  (interactive)
  (when themes
    (setq me/theme-known-themes themes))
  (dolist (theme me/theme-known-themes)
    (load-theme theme t t)))

(defun me/theme-cycle ()
  "Cycle through themes from `me/theme-known-themes' in succession.
End the loop with no theme enabled."
  (interactive)
  (let* ((current (car custom-enabled-themes))
         (next (if current
                   (cadr (memq current me/theme-known-themes))
...
Load the themes and fix some high-level faces to match my personal preferences.
...
(with-eval-after-load 'doom-themes
  ...
  (setq-default doom-nord-light-padded-modeline t)
  (let ((custom--inhibit-theme-enable nil))
    (doom-themes-set-faces 'doom-nord-light
      '(doom-visual-bell :background (doom-lighten red .5))
      '(doom-modeline-evil-emacs-state :foreground (doom-lighten violet .3))
...
#+BEGIN_SRC emacs-lisp
(defvar me/olivetti-whitelist-buffers
  '("*sratch*"))
...
(defhydra hydra-ui (:color pink)
 ...
 _t_ cycle           _b_ balance windows _-_ out             _n_ mode: %s`display-line-numbers
 ^^                  _m_ maximize frame  _=_ in              _N_ absolute: %s`display-line-numbers-current-absolute
 ^^                  ^^                  _0_ reset           ^^
 ^^                  ^^                  ^^                  ^^
 ^^                  ^^                  ^^                  ^^
")
...
(defhydra hydra-x (:color blue)
  (concat (me/hydra-heading "X" "Do" "Toggle") "
 _q_ quit            _r_ reset           _f_ floating        ^^
 ^^                  _R_ restart         ^^                  ^^
")
  ("q" nil)
...
(use-package projectile
  ...
  (setq-default
   ...
   projectile-known-projects-file (me/cache-concat ".projectile.eld"))
  :custom
...
(use-package amx
  :custom
  (amx-save-file (me/cache-concat ".amx"))
