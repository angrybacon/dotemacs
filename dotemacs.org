...
/=.emacs= file in a way that is both performance-oriented and, well, tidy. I created/
/it because I have over 80 packages that I use in Emacs, and things were getting/
...
/that Magit wraps and improves upon each and every Git command, it is complete/
/enough to allow even experienced Git users to perform almost all of their daily/
/version control tasks directly from within Emacs. While many fine Git clients/
/exist, only Magit and Git itself deserve to be called porcelains. [[https://magit.vc/about.html][(more)]]/
