...
While the =s-1= and =s-2= keybindings might look strange at first, their
respective command are swapped on purpose so that the former corresponds to my
left monitor, and the latter to my right monitor which is the primary one.
...
(use-package exwm
  ...
  (exwm-input-global-keys
   `((,(kbd "s-!") . me/exwm-read-shell-command)
     (,(kbd "s-1") . (lambda () (interactive) (exwm-workspace-switch-create 1)))
     (,(kbd "s-2") . (lambda () (interactive) (exwm-workspace-switch-create 0)))
...
#+BEGIN_SRC emacs-lisp
(defcustom me/theme-known-themes '(doom-nord doom-nord-light)
  "List of themes to take into account with `me/theme-cycle'.
See `custom-available-themes'.")

(defmacro me/theme-set-faces (theme &rest faces)
...
`custom--inhibit-theme-enable'."
  ...
  (with-eval-after-load 'doom-themes
    ...
    (let* ((active (eq theme (car custom-enabled-themes)))
           ;; FIXME: This value is not propagated to `doom-themes--build-face'
           (custom--inhibit-theme-enable (if active nil 'apply-only-user)))
      `(doom-themes-set-faces ',theme ,@faces))))
...
End the loop with no theme enabled."
  ...
  (let* ((current (car custom-enabled-themes))
    ...
    (if next
        (progn (load-theme next t) (message "%s" next))
      (message "No theme"))))
#+END_SRC
...
| TODO | Bind =s-(hjkl)= outside of EXWM |
...
#+BEGIN_SRC emacs-lisp
(defcustom me/olivetti-whitelist-buffers '("*sratch*")
  "List of buffers for which `olivetti-mode' should be enabled automatically.")

(defcustom me/olivetti-whitelist-modes '(Custom-mode
                                         Info-mode
                                         dired-mode
                                         erc-mode
