...
(use-package org
  ...
  :config
  (add-to-list 'safe-local-variable-values '(after-save-hook . (org-babel-tangle t)))
  (require 'ob-shell)
...
Use [[https://emacs-tree-sitter.github.io][Tree-sitter]] to handle more complex
syntax trees where the default alternatives lack the highlighting power.
